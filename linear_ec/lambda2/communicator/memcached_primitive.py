...
def reduce_batch(storage, vector, tmp_bucket, merged_bucket, num_workers, worker_index, cur_epoch, cur_batch):
    ...
    print("------worker {} epoch {} batch {}------ pass PUT".format(worker_index, cur_epoch, cur_batch))
    ...
    if worker_index == 0:
        ...
        print("------worker {} epoch {} batch {}------ pass WRITE MERGED".format(worker_index, cur_epoch, cur_batch))
    else:
        merged_file_key = postfix
        merged_data = storage.load_or_wait_v2(merged_file_key, merged_bucket, 0.1)
        if merged_data is None:
            print("------worker {} epoch {} batch {}------ merged_data {} is None".format(worker_index, cur_epoch, cur_batch))
            raise ValueError
